...
contract('Gaming', async (accounts) => {
  ...
  it('Should record player losses X', async() => {
    ...
    const gameRound = await gaming.winOrLose(10, true, 2,{
      from: player1,
    ...
    assert.equal(playerStats[1].toNumber(), 1, 'The player should have 1 loss .... this version')
  ...
  it('Should record player wins X', async() => {
    const gameRound = await gaming.winOrLose(10, false,2, {
      from: player1,
      value: web3.utils.toWei('1', 'ether')
    })
    const playerStats = await gaming.players(player1)
    assert.equal(playerStats[0].toNumber(), 1, 'The player should have 1 win.... this version')
  })
})
