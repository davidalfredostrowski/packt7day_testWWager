...
class App extends Component {
	...
	async loadBlockchainData(){
		...
		const web3 = new Web3(new Web3.providers.HttpProvider("http://ec2-54-184-204-198.us-west-2.compute.amazonaws.com:8545"))
		...
		contract.options.address = "0xB69a5e21d3cf4Bab31B4c7493e964e8E64EAA595";
...
contract.methods.fundGame().send({from: owner, value: web3.utils.toWei('10', 'ether')}).then((f) => console.log(f))	
		...
		// put in the user account address  ------ the short one , not the private key!!!!
  		...
  		let gameRound =   await contract.methods.winOrLose(10, true, web3.utils.toWei('10', 'ether')).send({from: player1 ,value: web3.utils.toWei('1', 'ether')}).then((f) => console.log(f))
		
	gameRound =   await contract.methods.winOrLose(10, false, web3.utils.toWei('10', 'ether')).call(console.log)
		//let gameRound =   await contract.methods.winOrLose(10, false).send({from: player1 }).then((f) => console.log(f))
		gameRound =   await contract.methods.winOrLose(10, false, web3.utils.toWei('10', 'ether')).call(console.log)
		...
		// truffle contract.deployed().then(function(contract)  {contract.wins.call().then(function(v) {console.log(v)})})		
    		...
    		//gameRound =   await contract.methods.winOrLose(1, false,  web3.utils.toWei('10', 'ether')).send({from: player1 ,value: web3.utils.toWei('1', 'ether') }).then((f) => console.log(f))

    		gameRound =   await contract.methods.winOrLose(1, true,  web3.utils.toWei('10', 'ether')).send({from: player1 ,value: web3.utils.toWei('1', 'ether') }).then((f) => console.log(f))
		gameRound =   await contract.methods.winOrLose(1, false,  web3.utils.toWei('10', 'ether')).call(console.log)
		...
		playerWins = await contract.methods.getWins().call(console.log)		
    		...
    		gameRound =   await contract.methods.winOrLose(5, true, web3.utils.toWei('10', 'ether')).send({from: player1,  value: web3.utils.toWei('1', 'ether') }).then((f) => console.log(f))
		gameRound =   await contract.methods.winOrLose(5, true, web3.utils.toWei('10', 'ether')).call(console.log)
		playerWins = await contract.methods.getWins().call(console.log)	
		...
		playerWins = await contract.methods.getWins().call(console.log)		
    		...
    		gameRound =   await contract.methods.winOrLose(5, false, web3.utils.toWei('10', 'ether')).send({from: player1,  value: web3.utils.toWei('1', 'ether') }).then((f) => console.log(f))
		gameRound =   await contract.methods.winOrLose(5, false, web3.utils.toWei('10', 'ether')).call(console.log)
			playerWins = await contract.methods.getWins().call(console.log)	
		...
		alert("contract.losses", playerLosses)
    		...
    		const endingBalance = await web3.eth.getBalance(player1)
		alert("endingBalance     " + String(endingBalance));

		const endingBalanceInEther = Number(web3.utils.fromWei(endingBalance, 'ether'))
		alert("endingBalanceInEther   " + String(endingBalanceInEther));
		console.log("endingBalance",endingBalance)
		console.log("endingBalanceInEther",endingBalanceInEther)
