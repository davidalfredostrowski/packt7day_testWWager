...
contract Gaming {
    ...
    //function winOrLose(uint display, bool guess) external payable returns (bool, uint, uint) {
    function winOrLose(uint display, bool guess, uint wager) external payable returns (bool, uint, uint, uint) {
	    ...
	    wins += 1;
            ...
            return (true, mysteryNumber_, msg.value, wager );
        } else if (isWinner == false) {
            /* Player lost */
            player.losses += 1;
	    losses += 1;
            emit RoundComplete(msg.value, display, mysteryNumber_, guess, "lost");
            return (false, mysteryNumber_,  msg.value, wager);
        }
    }
